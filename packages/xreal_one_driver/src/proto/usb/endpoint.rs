...
impl UsbEndpoint {
    ...
    > {
        ...
        let read_device = device.open_device(api)?;
        let write_device = device.open_device(api)?;
    ...
    pub async fn send_message<'req, Txn: UsbTransaction<'req>>(
        &self,
        ...
        self.command_tx
            ...
            .await
            .map_err(|_| anyhow!("USB write task stopped"))?;
        ...
        Ok(Response::deserialize_from(&response.payload)?)
    }
}
...
impl UsbWriteCommand {
    fn as_control_bytes(&self, current_request_id: u32) -> Result<[u8; 1024], anyhow::Error> {
